...
if __name__ == '__main__':
    ...
    with fiona.open(shp_path) as fc:
        for feature in fc:
            ...
            start_dt = datetime.combine(date.fromisoformat(props['Flood_Date']), start_time)
            ...
            running_spatial_extent = (min(running_spatial_extent[0], bbox[0]), max(running_spatial_extent[1], bbox[1]), min(running_spatial_extent[2], bbox[2]), max(running_spatial_extent[3], bbox[3]))
            ...
            with open('{}/{}/{}-usfimr.wkt'.format(root_path, fid, fid), 'w') as wkt_file:
                wkt_file.write(shapely_geom.wkt)

            with open('{}/{}/{}-usfimr.wkb'.format(root_path, fid, fid), 'wb') as wkb_file:
                wkb_file.write(shapely_geom.wkb)

            with open('{}/{}/{}-usfimr.geojson'.format(root_path, fid, fid), 'w') as geojson_file:
    ...
    root_collection.normalize_and_save(root_path, catalog_type=CatalogType.SELF_CONTAINED)
